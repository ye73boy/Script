...
!(async () => {
  ...
  console.log('京喜领88元红包\n' +
      ...
      '助力逻辑：先自己京东账号相互助力，如有剩余助力机会，则助力作者\n' +
      '温馨提示：如提示助力火爆，可尝试寻找京东客服')
  let res = await getAuthorShareCode('https://raw.githubusercontent.com/ye73boy/updateTeam/main/shareCodes/jxhb.json')
  if (!res) {
    $.http.get({url: 'https://cdn.jsdelivr.net/gh/ye73boy/updateTeam@main/shareCodes/jxhb.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/ye73boy/updateTeam@main/shareCodes/jxhb.json')
  }
